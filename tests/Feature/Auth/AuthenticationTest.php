...
test('users with two factor enabled are redirected to two factor challenge', function () {
    ...
    $response = $this->post(route('login', ['locale' => app()->getLocale()]), [
        'email' => $user->email,
        'password' => 'password',
    ]);
    $response->assertRedirect(route('two-factor.login', ['locale' => app()->getLocale()]));
...
test('users can not authenticate with invalid password', function () {
    $user = User::factory()->create();
    $this->post(route('login.store', ['locale' => app()->getLocale()]), [
        'email' => $user->email,
...
test('users can logout', function () {
    $user = User::factory()->create();
    $response = $this->actingAs($user)->post(route('logout', ['locale' => app()->getLocale()]));
    $this->assertGuest();
