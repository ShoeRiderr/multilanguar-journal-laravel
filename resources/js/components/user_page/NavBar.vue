    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between h-16 items-center">
        <div class="flex items-center space-x-8">
          <a class="flex items-center space-x-2 text-primary font-bold text-xl hover:cursor-pointer"
            :href="`/${locale}`">
        ...
        <div class="flex items-center space-x-4">
          <button
            class="p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-all hover:cursor-pointer"
          ...
          <div class="relative">
            <button
              class="dropdown-trigger flex items-center space-x-2 px-3 py-1.5 border border-slate-200 dark:border-slate-700 rounded-md hover:bg-slate-50 dark:hover:bg-slate-800 transition-all text-sm font-medium hover:cursor-pointer"
            ...
            <div
              :class="['dropdown-content', 'absolute', 'right-0', 'mt-2', 'w-40', 'rounded-lg', 'shadow-xl', 'py-1', 'overflow-hidden', 'z-50', 'bg-white', 'dark:bg-slate-800', 'border', 'border-slate-200', 'dark:border-slate-700', dropdownOpen ? '' : 'hidden']"
              id="langDropdown">
              <a v-for="language in languages" :key="language.code" href="#"
                @click.prevent="switchLanguage(language.code)" :class="[
          ...
          <Link v-if="$page.props.auth.user" class="flex w-full cursor-pointer" :href="logout({ locale: locale })"
          ...
          <template v-else>
            <div class="relative sm:hidden">
              ...
              <div :class="[
                ...
                <Link v-if="canRegister" :href="register({ locale: locale })"
                  class="flex items-center justify-between px-4 py-2 text-sm hover:bg-slate-50 dark:hover:bg-slate-700 rounded transition-all text-slate-600 dark:text-slate-300">
