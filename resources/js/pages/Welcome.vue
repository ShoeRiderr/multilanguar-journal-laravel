...
<template>
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <section
      class="text-center py-16 md:py-24 bg-gradient-to-b from-blue-50/50 to-transparent dark:from-blue-900/10 dark:to-transparent rounded-3xl mb-16">
      <div class="max-w-3xl mx-auto px-4">
        <h1 class="text-4xl md:text-6xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-6">
          {{ useTrans('welcome.title.part1') }} <span class="text-primary">{{ useTrans('welcome.title.part2') }}</span>
          {{ useTrans('welcome.title.part3') }}
        </h1>
        ...
        <div class="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-4">
          ...
          <button
            class="w-full sm:w-auto px-8 py-3.5 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 font-semibold rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-all shadow-md shadow-slate-500/10 hover:cursor-pointer">
    ...
    <div class="flex items-center justify-between mb-8 border-b border-slate-200 dark:border-slate-800 pb-4">
      ...
      <div class="flex items-center space-x-2 text-sm font-medium text-slate-500">
        ...
        <span class="bg-blue-100 dark:bg-blue-900/30 text-primary px-3 py-1 rounded-full text-xs">{{
          useTrans('posts.filtered_by.recently_published') }}</span>
      </div>
    </div>
    ...
    <div class="mt-16 text-center">
      <Link :href="`/${locale}/posts`"
        class="inline-flex items-center space-x-2 px-6 py-3 border border-slate-200 dark:border-slate-700 rounded-lg font-medium hover:bg-slate-100 dark:hover:bg-slate-800 transition-all hover:cursor-pointer">
        {{ useTrans('posts.all_articles') }}
